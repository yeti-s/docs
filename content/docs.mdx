...
또한 .mdx 파일에서 사용할 이미지는 해당 파일과 동일한 위치에 assets/{file_name} 폴더에 저장한다.
```
|-- content
    |-- file1.mdx
    |-- file1
...
### 
위 수식 추가 작업을 진행하며 gatsby-theme-document 모듈 내부를 수정하였으나 이는 다른 환경에서 Git clone 하면 해당 부분을 다시 수정해야 하는 번거로움이 있다.
그래서 Git에 변경사항이 저장되고 또 template의 customize를 가능하게 하도록 해당 모듈을 root directory로 이식하는 작업을 진행하려 한다.
...
![](assets/docs/00_change_template_style.PNG.PNG)